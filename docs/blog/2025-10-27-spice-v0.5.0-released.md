...
title: Spice v0.5.0 - Graph System & Human-in-the-Loop 🕸️
authors: [spice-team]
tags: [release, graph, orchestration, hitl, middleware, checkpoint, microsoft-af, breaking-change]
---

# Spice v0.5.0: Graph System & Human-in-the-Loop 🕸️
...
- 🕸️ **Graph System** - DAG-based orchestration (Swarm/Flow → Graph)
- 🔄 **Middleware** - Intercept execution for logging, metrics, observability
- 💾 **Checkpointing** - Save and resume graph execution
- 👤 **HITL Support** - Pause graphs for human approval
- 🤝 **Agent Handoff** - Seamless agent → human transfers
- 📚 **4900+ lines of docs** - Complete guides and examples
- ✅ **24 new tests** - Comprehensive test coverage
...
- ❌ **Limited flexibility** - Linear or predefined strategies
- ❌ **No state management** - Couldn't pause and resume
- ❌ **No human oversight** - All-AI or manual orchestration
- ❌ **Poor composability** - Hard to build complex workflows
...
- ✅ **DAG execution** - Directed Acyclic Graphs for any workflow
- ✅ **Conditional routing** - Route based on runtime data
- ✅ **State management** - Checkpoint and resume execution
- ✅ **Human-in-the-Loop** - Pause for human approval
- ✅ **Better composability** - Combine simple graphs into complex systems
- ✅ **Industry-aligned** - Matches Microsoft AF design philosophy

---

## Feature Deep Dive

### 1. Graph System - The New Orchestration Core 🕸️
...
### 2. Middleware - Cross-Cutting Concerns Made Easy 🔄
...
class LoggingMiddleware : Middleware {
    override suspend fun onGraphStart(context: GraphContext) {
        println("🚀 Starting graph: ${context.graphId}")
    }
    ...
    override suspend fun onNodeExecute(
        ...
        println("✅ Node ${node.id} completed in ${duration}ms")
    ...
    override suspend fun onNodeError(
        ...
        println("❌ Node ${node.id} failed: ${error.message}")
    ...
    override suspend fun onGraphFinish(
        ...
        println("🏁 Graph ${context.graphId} finished")
    }
}
...
class RetryMiddleware(val maxRetries: Int = 3) : Middleware {
    override suspend fun onNodeError(
        ...
        return if (attempt < maxRetries) {
            ...
            println("🔄 Retrying node ${node.id} (attempt ${attempt + 1}/$maxRetries)")
            ErrorAction.RETRY
        } else {
            println("💀 Max retries reached for node ${node.id}")
...
- 🎯 **Custom Behavior** - Inject any cross-cutting concern

---

### 3. Checkpointing - Fault Tolerance & Long-Running Workflows 💾
...
- ⏱️ **Long-Running Workflows** - Execute workflows over days/weeks
- 👤 **Human Approval** - Wait for human input (HITL)
- 📊 **Audit Trail** - Track execution history
- 🔍 **Debugging** - Inspect state at any point

---

### 4. Human-in-the-Loop (HITL) - The Game Changer 👤
...
- 💰 **Financial Approvals** - Multi-level transaction authorization
- ⚖️ **Legal Review** - Review contracts and legal documents
- 🏥 **Healthcare** - Review AI diagnosis recommendations
- 🔒 **Compliance** - Meet regulatory requirements for human oversight
- 🎯 **Quality Control** - Ensure accuracy before production deployment

**HITL Features**:
- ✅ **Choice-based** - Present options to humans
- ✅ **Free text** - Collect open-ended feedback
- ✅ **Validators** - Ensure response quality
- ✅ **Timeouts** - Auto-fail if no response
- ✅ **Multi-stage** - Sequential approval chains
- ✅ **Conditional routing** - Branch based on human choice

---

### 5. Agent Handoff - Different from HITL 🤝
...
|-------|-------|-----------------|
| `swarm {}` | `graph {}` | ✅ Yes - Replace |
| `flow {}` | `graph {}` | ✅ Yes - Replace |
| Execution | `GraphRunner` | ✅ Yes - Use runner |
| Context | `ctx.state["key"]` | ✅ Yes - Update access |

### What Didn't Change

| Component | Status | Migration Needed |
|-----------|--------|------------------|
| Agent interface | ✅ Same | ❌ No |
| Tool interface | ✅ Same | ❌ No |
| Comm class | ✅ Same | ❌ No |
| AgentContext | ✅ Same | ❌ No |
| SpiceResult | ✅ Same | ❌ No |
...
- ✅ HITL for human review
- ✅ HITL for manager approval
...
- 📚 **Full Documentation**: https://docs.spice.noailabs.io
- 🐙 **GitHub**: https://github.com/no-ai-labs/spice
- 📋 **Changelog**: [CHANGELOG.md](https://github.com/no-ai-labs/spice/blob/main/CHANGELOG.md)
- 📝 **Release Notes**: [RELEASE_NOTES_v0.5.0.md](https://github.com/no-ai-labs/spice/blob/main/RELEASE_NOTES_v0.5.0.md)
- 🔄 **Migration Guide**: [docs/roadmap/migration-guide.md](https://docs.spice.noailabs.io/docs/roadmap/migration-guide)
...
We've added **24 new tests** for 0.5.0 features, all passing:
...
- ✅ Response validators (accept/reject)
- ✅ Timeout handling (accept/reject)
...
All 24 new 0.5.0 tests: **100% passing** ✅
...
- 📡 **Streaming Support** - Stream graph execution results
- 🔗 **Multi-Graph Composition** - Compose graphs together
- 📊 **Advanced Observability** - OpenTelemetry integration
- 🔄 **Graph Versioning** - Version and migrate graphs
- 🎯 **Dynamic Graph Construction** - Build graphs at runtime
...
- 💬 **Community Feedback** - Real-world use cases that shaped these features
- 🔧 **Core Team** - Months of architecture, implementation, and testing
- 📚 **Documentation Contributors** - 4900+ lines of comprehensive docs
...
Your feedback directly shaped 0.5.0! 🎉
