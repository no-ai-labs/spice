...
val chatRequest = ChatCompletionRequest(
    model = ModelId("gpt-5"),
    messages = messages,
    functions = listOf(openAISpec)  // ✅ Ready to go
)
```
...
// Your OpenAI function spec (version 2) 😱
...
**Perfect OpenAI format. Zero manual work.** ✨
...
**Perfect for:**
- ✅ Production APIs with strict contracts
...
class AdaptiveRouter(
    ...
    suspend fun execute(userMessage: String): String {
        // Simple queries → Execute locally via Spice (fast, free)
        if (isSimpleQuery(userMessage)) {
            return executeLocally(userMessage)
        }

        // Complex reasoning → Use OpenAI with tools (powerful)
        val openAIFunctions = tools.map { it.toOpenAIFunctionSpec() }
...
router.execute("Calculate 42 * 1337")  // → Spice (local, instant)
router.execute("Analyze Q3 sales and suggest improvements")  // → OpenAI + Spice tools
```
...
fun `test OpenAI function calling integration`() = runTest {
    ...
    // ✅ Perfect! Tool works both locally and with OpenAI
}
```
...
Spice parameter types → OpenAI JSON Schema types:
...
ToolSchema(
    ...
    parameters = mapOf(
        ...
        "timeout" to ParameterSchema(
            ...
            required = false,
            default = JsonPrimitive(30)  // ✅ Included in OpenAI spec
        )
    )
)
