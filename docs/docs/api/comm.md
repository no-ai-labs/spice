...
val image = MediaItem(
    ...
    metadata = mapOf(
        ...
        "format" to "png",
        "captured_at" to "2024-01-15T10:30:00Z"
    )
)
```
...
**Signatures:**
```kotlin
fun withData(key: String, value: String): Comm
fun withData(vararg pairs: Pair<String, String>): Comm
```
...
| `duration_ms` | Processing duration | `1250` |
| `model` | LLM model used | `gpt-4`, `claude-3-sonnet` |
| `tokens` | Token count | `150` |
| `cost` | Estimated cost | `0.003` |
| `error_code` | Error code | `RATE_LIMIT`, `INVALID_INPUT` |
| `retry_count` | Retry attempts | `2` |
| `tool_name` | Tool name (auto-set) | `calculator` |
| `tool_params` | Tool params (auto-set) | `{a: 2, b: 2}` |
| `user_id` | User identifier | `user-123` |
| `session_id` | Session identifier | `sess-456` |
| `trace_id` | Distributed trace ID | `abc123xyz` |
