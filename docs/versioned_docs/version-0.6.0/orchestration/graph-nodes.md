...
override suspend fun run(ctx: NodeContext): SpiceResult<NodeResult> {
    ...
    val comm = Comm(
        ...
        context = ctx.context,  // ✨ Auto-propagates ExecutionContext
    ...
    return agent.processComm(comm)  // SpiceResult<Comm>
        .map { response ->
            // 5️⃣ Store full Comm in metadata for next node
            NodeResult.fromContext(
                ...
                additional = mapOf(
                    ...
                    "_previousComm" to response  // GraphRunner will put in state
                )
            )
...
class FullCommAgentNode(
    ...
    override suspend fun run(ctx: NodeContext): SpiceResult<NodeResult> {
        ...
        return agent.processComm(previousComm)
            .map { response ->
                // ✅ Store full Comm in metadata for next node
                NodeResult.fromContext(
                    ctx, 
                    data = response.content,
                    additional = mapOf("_previousComm" to response)
                )
            }
    }
}
...
class DelayNode(
    ...
    override suspend fun run(ctx: NodeContext): SpiceResult<NodeResult> {
        return SpiceResult.catchingSuspend {
            ...
            NodeResult.fromContext(
                ctx,
                data = "Delayed for ${delayMs}ms"  // 🔥 Always preserves context!
            )
        }
    }
}
    ...
    override suspend fun run(ctx: NodeContext): SpiceResult<NodeResult> {
        return SpiceResult.catching {
            ...
            NodeResult.fromContext(
                ctx,
                data = result,
                additional = mapOf(  // 🔥 Preserve existing context!
...
val node = object : Node {
    ...
    override suspend fun run(ctx: NodeContext): SpiceResult<NodeResult> {
        ...
        return SpiceResult.success(
            NodeResult.fromContext(ctx, data = "processed")  // 🔥 Always preserve context!
        )
    }
}
```
...
val node = object : Node {
    ...
    override suspend fun run(ctx: NodeContext): SpiceResult<NodeResult> {
        ...
        // GraphRunner will propagate these to state automatically
    }
}
```
